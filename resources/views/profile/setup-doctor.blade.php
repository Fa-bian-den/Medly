...
<div class="container">
  ...
  <form action="{{ route('profile.setup.store') }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="mb-3">
      <label for="carnet_minsa">Carnet MINSA</label>
      <input id="carnet_minsa" name="carnet_minsa" value="{{ old('carnet_minsa', $user->carnet_minsa ?? '') }}" class="form-control">
    </div>

    <div class="mb-3">
      <label for="professional_details">Detalles profesionales</label>
      <textarea id="professional_details" name="professional_details" class="form-control">{{ old('professional_details', optional($user->doctorProfile)->professional_details) }}</textarea>
    </div>

    <div class="mb-3">
      <label for="specialties">Especialidades (IDs separados)</label>
      <input id="specialties" name="specialties" value="{{ old('specialties') }}" class="form-control" placeholder="Ej: 1,2,3">
      <small class="form-text text-muted">Si tu UI no tiene select múltiple, puedes aceptar CSV y parsearlo en backend.</small>
    </div>

    <div class="mb-3">
      <label for="documents">Documentos (pdf/jpg/png)</label>
      <input id="documents" type="file" name="documents[]" multiple class="form-control">
    </div>
